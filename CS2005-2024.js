...
var ProvinceList = ['Aomen','Xianggang',
...
function calculate_FPAR(ndvi, landcover) {
  ...
  NDVI_max = NDVI_max.where(landCover.eq(1), 0.7).where(landCover.eq(2), 0.70)
                     .where(landCover.eq(3), 0.68).where(landCover.eq(4), 0.70)
                     .where(landCover.eq(7), 0.5).where(landCover.eq(9), 0.68);
...
function calculate_LUE(t, et, pet, landCover, year, t_opt) {
  ...
  ε_max = ε_max.where(landCover.eq(2), 0.692).where(landCover.eq(3), 0.429)
               .where(landCover.eq(5), 0).where(landCover.eq(6), 0)
               .where(landCover.eq(7), 0.2).where(landCover.eq(8), 0);
...
for (var year = startYear; year <= endYear; year++) {
  ...
  for (var j = 0; j < ProvinceList.length; j++) {
    ...
    for (var i = 1; i <= 12; i++) {
      ...
      var Rh_month = ee.Image(0.22).multiply((t.multiply(0.0912).exp()).add((p.multiply(0.3145).add(1)).log()))
        .multiply(30).multiply(0.465).multiply(LUC); 
    ...
    // 6. 固碳量转换 (t/hm2/year)
    var annualGuTan = ee.Image(44/12).multiply(annualNEP).multiply(0.01).rename('GuTan');

    /************************************************************
     * 4. 导出逻辑
     ************************************************************/
    Export.image.toDrive({
      image: annualGuTan.clip(table),
      description: 'GuTan_' + loc + '_' + year,
      fileNamePrefix: 'GuTan_' + loc + '_' + year,
      folder: 'GuTan_Annual_' + year + '_30M',
      scale: scale,
      region: table,
      crs: "EPSG:4326",
      maxPixels: 1e13
...
print('Carbon Sequestration calculation finished! Check Tasks.');